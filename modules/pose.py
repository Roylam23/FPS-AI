        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)),
                         (int(x_b), int(y_b)), Pose.color, 2)

    def draw_spec(self, img,aim):
        assert self.keypoints.shape == (Pose.num_kpts, 2)
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            kpt_a_id = BODY_PARTS_KPT_IDS[part_id][0]
            global_kpt_a_id = self.keypoints[kpt_a_id, 0]
            if global_kpt_a_id != -1:
                ...
                if part_id == selected_part:
                    ...
                    y = (int(y_a)+top)-oriH
                    if x<= px and x>= -px:
                        cv2.line(img, (int(x_a), int(y_a)),(int(width/2),int(height/2)), (255, 255, 255), 1)
                    else:
                        print('Nearby Target In X: '+str(x)+' Y: '+str(y)+" ", end='\r')
                        sys.stdout.flush()
                if aim:
                    threading.Thread(target=shoot,args=(x_a,y_a,part_id)).start()
                cv2.circle(img, (int(x_a), int(y_a)), 3, Pose.cir_color, -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 3, Pose.cir_color, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)),
                         (int(x_b), int(y_b)), Pose.line_color, 2)



def cir(img,x,y,color):
    cv2.circle(img, (int(x), int(y)), 3, color, -1)

selected_part = 0
width = int(700)
height = int(500)
top = int((1425 - height) / 2)
left = int((2560 - width) / 2)
oriW = int(2560/2)
oriH = int(1440/2)
px = 95
py = 175

def shoot(x_a,y_a,part_id):
        if part_id == selected_part:
            x = (int(x_a)+left)-oriW
            y = (int(y_a)+top)-oriH
            # threading.Thread(target=movement,args=(x,y)).start()
            movement(x,y)
            if x < 50 and x > -50 and y > -100 and y < 50:
                mouse.click('left')
            return False

def movement(x,y):
    if x<= px and x>= -px:
            ctypes.windll.user32.mouse_event(0x0001, int(x), 0, 0, 0)
            if x < 30 and x > -30 and y > -100 and y < 50:
                print('Trigger Shoot Action                   ', end='\r')
                sys.stdout.flush()
            else:
                print('Moving to Target X: '+str(x)+' Y: '+str(y)+" ", end='\r')
                sys.stdout.flush()
    if y<= py and y>= -py:
            ctypes.windll.user32.mouse_event(0x0001, 0, int(y*0.35), 0, 0)
